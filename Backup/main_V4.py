...
if not orders_df.empty:
    ...
    if nom_counts[nom_counts > 1].any():
        st.warning("⚠️ Certains noms ont des données incohérentes. Seuls les noms uniques sont utilisés pour l'autoremplissage.")

    client_info_map = clients_df_filtered.set_index("Nom").to_dict("index")

    # === Table éditable ===
    st.subheader("📝 Édition des commandes de la semaine")
    base_df = df_semaine.drop(columns=["date_livraison"]).copy()

    base_df["order_number"] = base_df["order_number"].astype(str)


    # Numéro de commande max pour génération auto
    max_order_number = (
        base_df["order_number"].dropna()
        .astype(str)
        .str.extract(r"(\d+)")
        .astype(float)
        .max()[0]
        if not base_df["order_number"].dropna().empty else 0
    )
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            "index_ligne": st.column_config.NumberColumn(
                "Ligne #", required=True, disabled=True
            ),
    ...
    # Mise à jour automatique post-édition
